...
# encoding: utf-8
"""
official 2d tracking evaluator from kitti
...
__main__ code converts numpy-saved tracker results to kitti text format
    then runs this evaluator code
"""
...
class Mail:
    ...
    def finalize(self,success,benchmark,sha_key,mailaddress=None):
        if success:
            print("Results for %s (benchmark: %s) sucessfully created" %
                  (benchmark,sha_key))
        else:
            print("Creating results for %s (benchmark: %s) failed" %
                  (benchmark,sha_key))
...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path="/home/m2/Data/kitti/tracking_gt", min_overlap=0.5,
                 max_truncation = 0, min_height = 25, max_occlusion = 2, mail=None,
                 cls="car"):
        # get number of sequences and
        # get number of frames per sequence from test mapping
        # (created while extracting the benchmark)
        filename_test_mapping = "evaluate_tracking2.seqmap"
        self.n_frames         = []
        self.sequence_name    = []
        ...
        # data and parameter
        self.gt_path           = gt_path#os.path.join(gt_path, "label_02")
        self.t_sha             = t_sha
        self.t_path            = t_sha#os.path.join(t_sha, "data")
    ...
    def loadTracker(self):
        ...
        return self._loadData(self.t_path, cls=self.cls, loading_groundtruth=False)
    ...
    def _loadData(self, root_dir, cls, min_score=-1000, loading_groundtruth=False):
        ...
        for seq, s_name in enumerate(self.sequence_name):
            ...
            for line in f.split("\n"):
                ...
                if len(fields) < 10:
                    print("Error in fields @ {:s}".format(filename))
                    print(line)
                    raise Exception
                ...
                if (not loading_groundtruth and
                    eval_2d is True and
            ...
            for f in range(len(seq_gt)):
                ...
                for tt in t:
                    ...
                    if (((self.cls=="car" and tt.obj_type=="van") or
                         (self.cls=="pedestrian" and tt.obj_type=="person_sitting") or
                ...
                for gg in g:
                    if gg.tracker < 0:
                        if (gg.occlusion>self.max_occlusion or
    ...
    def saveToStats(self):
        ...
        dump = open(filename, "w+")
        print("MOTA", "MOTP", "MOTAL", "MODA", "MODP", "recall", "precision",
              "F1", "FAR", file=dump)
        print("MT", "PT", "ML", "tp", "fp", "fn", "id_switches", "fragments",
              file=dump)
...
if __name__ == "__main__":
    ...
    for scene_idx in scenes:
        ...
        for file_idx in range(startfileidx, endfileidx):
            ground = np.load(ground_files.format(scene_idx, file_idx))
            ests = np.load(inestimatefiles.format(scene_idx, file_idx))
            dontcares = {file_idx:[]}
            out = formatForKittiScoreTracking(ests[:,:5],ests[:,6],ests[:,5],
                                              file_idx, ground, calib_projection,
                                              imgshape, dontcares)
            if out!='':
                fullout.append(out)
        fullout = '\n'.join(fullout)
        with open(outestfiles.format(scene_idx), "w") as fd:
            fd.write(fullout)
    
    
    mail = Mail("")
    e = trackingEvaluation(t_sha=
                    "/home/m2/Data/kitti/estimates/trackingresults000/kitti/",
                           mail=mail,cls="car")
